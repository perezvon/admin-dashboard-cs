...
import {sortCollection, getCompanyInfo, getStoreID, validateEmail} from './global'
...
class App extends React.Component {
  ...
  render() {
    ...
    if (!_.isEmpty(this.state.data) && !_.isEmpty(this.state.customers) && this.state.loading === false) {
      ...
      let approvedOrders = this.state.approve ? data.filter(i => i.status === 'P') : data.filter(i => i.status !== 'I')
